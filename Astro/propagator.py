...
## Initialise Atmospheric Model (via SWAMI)
mcm = swami.MCM()
...
def constructWind(state):
    # Function to input ECEF coordinates into wind coordinates
    ## INPUTS
    # state - a 6 element numpy array describing vehicle state in ECEF frame
    ## OUTPUTS
    # altitude (m) - radial distance from surface
    # velocity (m/s) - vehicle speed along flight path
    # flight_path (deg) - angle from the horizon of vehicle
    # heading (deg) - heading angle w/ respect to North, +ve values eastward, -ve westward
    ...
    # Altitude as radial distance minus the Earth's radius
    altitude = np.linalg.norm(np.linalg.norm(x) - rad)

    # Define radial direction vector
    unit_x = x / np.linalg.norm(x)

    # Unit vector for in-plane direction
    ...
    # Ensure compliance with east/west scheme
...
def satEoS(t, state):
    ...
    # Bool to track BC calculations
    recalculate = 0

    # Get satellite BC...
    if 'Checkpoint_state' in cfg['Propagator'] and not t == 0.0: # Have we written a checkpoint?
        cfg['Options']['Load_mesh'] = 'True' # If so TITAN has initialised the mesh
        # Perform some string manipulation to get vector out of cfg file
        ...
        # If we wrote the checkpoint this iter, recalculate BC
...
def getDensity(x, t, cfg):
    ...
    # x - xyz position vector in ITRF space (m)
    ...
    # Pull atmosphere criterion from cfg
...
def computeBC(state, cfg, recalculate):
    ...
    if recalculate or (not 'BCD' or not 'BCL' or not 'BCY' in cfg['Ballistics']):
        ...
        if cfg['Ballistics']['Method'] == 'NAIVE':
            ...
            BCD = (float(cfg['Ballistics']['Drag_coefficient']) * float(cfg['Ballistics']['Reference_area'])) / float(
                cfg['Ballistics']['BCMass'])

            BCL = (float(cfg['Ballistics']['Lift_coefficient']) * float(cfg['Ballistics']['Reference_area'])) / float(
                cfg['Ballistics']['BCMass'])

            BCY = (float(cfg['Ballistics']['Side_coefficient']) * float(cfg['Ballistics']['Reference_area'])) / float(
                cfg['Ballistics']['BCMass'])
...
def titanBC(state, cfg, fidelitySwitch):
    ...
    mpiRank = dragCfg['MPI']['Rank'] if 'Rank' in cfg['MPI'] else '0'

    if not os.path.exists(dir + '/DTOURtemp'):
        os.makedirs(dir + '/DTOURtemp')

    dragCfg['Options']['Output_folder'] = dir + '/DTOURtemp/dragData'+mpiRank
    ...
    filepath = dir + '/DTOURtemp/dragTemp' + str(mpiRank) + '.cfg'
    ...
    data_csv = dir + '/DTOURtemp/dragData'+str(mpiRank)+'/Data/data.csv'
...
def runPropagator(state, forecast, cfgfile):
    ...
    # Begin sim
    mpiRank=cfg['MPI']['Rank'] if 'Rank' in cfg['MPI'] else '0'
    if int(mpiRank)==0:
    ...
    hSeries = np.array((np.linalg.norm(x) - earth_rad) / 1000,
                       (np.linalg.norm(propagator.y[:3, -1]) - earth_rad) / 1000)
    ...
    while propagator.t[-1] < forecast:
        ...
        propagator = integrate.solve_ivp(fun=satEoS, t_span=(propagator.t[-1], propagator.t[-1] + t_max), y0=state_new,
                                         method='RK45')  # call propagator
        ...
        v = propagator.y[3:, -1]
        x = propagator.y[:3, -1]
        nonOrb = np.linalg.norm(
            np.cross((propagator.y[:3, -1] / np.linalg.norm(propagator.y[:3, -1])), propagator.y[3:, -1]))
        ...
        if h < float(cfg['Propagator']['Critical_altitude']):
            print('Low altitude, object has \'entered\' atmosphere h=', h, 'km total duration of ',
                  dt.timedelta(seconds=propagator.t[-1]))
            entryFlag = 1
    ...
    if 'Write_data' in cfg['Propagator']:
        ...
        pd.DataFrame(stateSeries).to_csv(
            str('data/position_' + dt.datetime.now().strftime('%Y_%m_%d__%H_%M_%S') + '.csv'))
