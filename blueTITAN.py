...
class MyProblem(BLUEProblem):
    ...
    def evaluate(self, ls, samples):
        ...
        TITANcfg=cfg
        meshvector=["sphere.stl","decimated.stl","doubledecimated.stl"]
        for i in range(L):
            ...
            TITANcfg['Objects']['Primary']="[NAME = " + meshvector[ls[i]] +", MATERIAL = Unittest, TYPE = Primitive, FENICS_ID = -1]"
            ...
            allSamples=sparse.vstack((allSamples,np.zeros(shape=(1,6))),format='lil')
            allSamples[-1,i] = lat
